...
class SubEventRepository {
  ...
  Future<SubEventModel> subEventWithImage(_attachment, _category, _msg) async {
    String token = "";
  ...
  Future<SubEventModel> subEventWithoutImage(_category, _msg) async {
    String token = "";
    Future<UserModel> getUserData() => UserViewViewModel().getUser();
    getUserData().then((value) async {
      token = value.access.toString();
    ...
    try {
      dynamic response =
          await _apiServices.getPostResponseWithAuthMultipartDataWithoutImage(
              AppUrl.postSubEvent(), _category, _msg, obj_id, token);
      return response = SubEventModel.fromJson(response);
    } catch (e) {
      throw e;
    }
